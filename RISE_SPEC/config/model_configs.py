...
# Dataset-specific weights and class counts per model.
# These override the defaults in MODEL_CONFIGS when a dataset is specified.
MODEL_DATASET_OVERRIDES = {
    'resnet50': {
        'esc50':        {'num_classes': 50, 'weights_path': 'checkpoints/resnet50_esc50.pt'},
        'urbansound8k': {'num_classes': 10, 'weights_path': 'checkpoints/resnet50_urbansound8k.pth'},
    },
    'htsat': {
        'esc50':        {'num_classes': 50, 'weights_path': 'checkpoints/HTSAT.ckpt'},
        'urbansound8k': {'num_classes': 10, 'weights_path': 'checkpoints/htsat_us8k.pth'},
    },
    'wav2vec2': {
        'esc50':        {'num_classes': 50, 'weights_path': 'checkpoints/wav2vec2_esc50.pt'},
        'urbansound8k': {'num_classes': 10, 'weights_path': 'checkpoints/best_model_wav2vec2_us8k.pt'},
    },
}


def get_model_config_for_dataset(model_type: str, dataset_name: str) -> dict:
    """Get model config merged with dataset-specific num_classes and weights_path.

    Args:
        model_type: 'resnet50', 'htsat', or 'wav2vec2'
        dataset_name: 'esc50' or 'urbansound8k'

    Returns:
        dict: Model config with dataset-appropriate num_classes and weights_path.
    """
    config = get_model_config(model_type)
...
def get_dataset_config(dataset_name):
    ...
    Returns:
        dict with keys: num_classes, default_spectrogram_dir, default_audio_dir
    """
